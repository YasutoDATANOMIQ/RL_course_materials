...
class PendulumObserver(Observer):
    ...
    def step(self, action):
        n_state, reward, done, info, _ = self._env.step([action])
...
class PolicyGradientContinuousTrainer(Trainer):
    ...
    def begin_train(self, episode, agent):
        actor_optimizer = K.optimizers.legacy.Adam(lr=0.001, clipnorm=1.0)
        critic_optimizer = K.optimizers.legacy.Adam(lr=0.001, clipnorm=1.0)
...
def main(play):
    env = PendulumObserver(gym.make("Pendulum-v1", render_mode="human"))
